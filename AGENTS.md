...
- LocalQwen 的候选生成、表达式审查、失败聚类属于 `local_generation_or_review`，只能作为本地生成/复核辅助；不得按 `input_chars - output_chars` 计入上下文节省。只有上下文蒸馏、文件摘要、失败日志提取、上下文选择等 `context_saving_eligible` 工具才可进入压缩收益口径，且仍需扣除后续回源字符。
